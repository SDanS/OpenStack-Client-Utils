...
sub new {
    ...
    my (
        ...
        $use_env_auth, $force_reauth, $use_overrides,
    );

    # Defaults to 1;
    ( $args{'use_overrides'} = \$use_overrides )    #
      ? $use_overrides = 1 : $use_overrides = 0;
    ...
    $debug = $args{'debug'};
    %args  = $args{'use_env_auth'} ? $self->auth_from_env(%args) : %args;

    # tidyoff
    print "\n" . '%args prior to auth_boilerplate():' . "\n" if $debug; print Dumper \%args if $debug;
    # tidyon

    %args = auth_boilerplate(%args);
