...
sub set_auth_tracking {
    ...
    print __LINE__ . ": auth: \n";
    print Dumper $auth; if $debug;
    print "\ntracking_keys:  \n"; if $debug;
    print Dumper $tracking_keys; if $debug;
...
sub auth_from_env {
    ...
    $auth_args->{'version'} = $ENV{'OS_IDENTITY_API_VERSION'} ? $ENV{'OS_IDENTITY_API_VERSION'} : $auth->{'version'};
...
sub value_from_response {
    ...
    print "->{\"$_\"}" foreach @$sub_args if $debug;
    print "\n" if $debug;
...
sub get_response_attribute {
    ...
    my $attr        = ( exists $response->{$sub_arg_arr} && !ref $response->{$sub_arg_arr} ) ? return $response->{$sub_arg_arr} : scalar $sub_arg_arr->[0] == 1 ? return $response->{ $sub_arg_arr->[0] } : ref $response->{$sub_arg_arr} =~ /ARRAY|HASH/ ? walk_ref_path( $response, $sub_arg_arr ) : value_from_response( $response, $sub_arg_arr );
...
sub walk_ref_path {
    ...
    elsif ( exists $res_ref_part->{ $next_steps[0] }
...
sub arrays_of_hashes {
    ...
    foreach my $stepped_data ($#$res_data) {
        if ( exists $stepped_data->{ $nested_key_path[0] }
...
sub auth_boilerplate {
    ...
    $fwd_args{'token'} = $args->{'token'}
      ...
      ### Don't fetch tokens if force_reauth is true.
      : ( !$args->{'force_reauth'} && $self->{'auth'} && $self->{"auth"}->token() ) ? $self->{"auth"}->token()
      :                                                                               undef;

    print "Inside token helper (auth_boilerplate())\nToken: " . $fwd_args{'token'} || '' if $debug;
